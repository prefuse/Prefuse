...
import prefuse.visual.UserLabel;
import prefuse.visual.VisualItem;
import prefuse.visual.expression.VisiblePredicate;
import prefuse.visual.sort.ItemSorter;

/**
 * <p>User interface component that provides an interactive view onto a
 ...
 * from which it pulls the items to visualize. </p>
 *
 * <p>To control which {@link prefuse.visual.VisualItem} instances are drawn,
 * the Display also maintains an optional
 * {@link prefuse.data.expression.Predicate} for filtering items. The drawing
 * order of items is controlled by an {@link prefuse.visual.sort.ItemSorter}
 * instance, which calculates a score for each item. Items with higher scores
 * are drawn later, and hence on top of lower scoring items. </p>
 *
 * <p>The {@link prefuse.controls.Control Control} interface provides the user
 * interface callbacks for supporting interaction. The {@link prefuse.controls}
 * package contains a number of pre-built
 * <code>Control</code> implementations for common interactions.</p>
 *
 * <p>The Display class also supports arbitrary graphics transforms through the
 ...
 * and zooming of the presentation space.</p>
 *
 * <p>Additionally, each Display instance also supports use of a text editor to
...
public class Display extends JComponent {
     ...
     * are <ul><li><b>ctrl D</b> - Toggle debug info display</li> <li><b>ctrl
     * H</b> - Toggle high quality rendering</li> <li><b>ctrl E</b> - Export
     * display view to an image file</li></ul> Subclasses can override this
     * method to prevent these commands from being set. Additional commands can
     ...
     * parsed by the {@link prefuse.data.expression.parser.ExpressionParser}. If
     * the parse fails or does not result in a
     * {@link prefuse.data.expression.Predicate} instance, an exception will be
     * thrown.
     */
     ...
     *
     * @param image the background Image. If a null value is provided, than no
     * background image will be shown.
     * @param fixed true if the background image should stay in a fixed
     * position, invariant to panning, zooming, or rotation; false if the image
     ...
     * Set a background image for this display.
     *
     * @param location a location String of where to retrieve the image file
     * from. Uses {@link prefuse.util.io.IOLib#urlFromString(String)} to resolve
     * the String. If a null value is provided, than no background image will be
     * shown.
     * @param fixed true if the background image should stay in a fixed
     * position, invariant to panning, zooming, or rotation; false if the image
     * should be subject to view transforms
     ...
     * try disabling damage/redraw.
     *
     * @param b true to enable damage/redraw optimizations, false otherwise (in
     ...
     * @param abs a reference to put the result in. If this is the same object
     * as the screen co-ordinate, it will be overridden safely. If this value is
    ...
    /**
     * Gets the screen co-ordinate corresponding to the given absolute
     * co-ordinate.
     *
     * @param absolute the absolute co-ordinate to transform
     * @param abs a reference to put the result in. If this is the same object
     * as the screen co-ordinate, it will be overridden safely. If this value is
     * null, a new Point2D instance will be created and returned.
     * @return the point in absolute co-ordinates
     */
    public Point2D getScreenCoordinate(Point2D absolute, Point2D abs) {
        return m_transform.transform(absolute, abs);
    }
     ...
     * x-dimension, so be careful when using this value in rare non-uniform
     * scaling cases.
     */
     ...
     *
     * @param p the anchor point for the zoom, in absolute (i.e. item-space)
     * co-ordinates
     * @param scale the amount to zoom by
     */
     ...
     *
     * @param p the anchor point for the rotation, in absolute (i.e. item-space)
     * co-ordinates
