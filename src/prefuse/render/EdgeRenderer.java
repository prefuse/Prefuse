...
public class EdgeRenderer extends AbstractShapeRenderer {
    ...
    protected MixedGraphEdgeType mixedEdgeType = MixedGraphEdgeType.Default;
    final double offsetStrength = 1d / 4d;
    ...
    public EdgeRenderer(int edgeType, int arrowType, MixedGraphEdgeType mixedEdgeType) {
        this();
        setEdgeType(edgeType);
        setArrowType(arrowType);
        this.mixedEdgeType = mixedEdgeType;
    }
    ...
    protected Shape getRawShape(VisualItem item) {
        ...
        if (e.isDirected() && m_edgeArrow != Constants.EDGE_ARROW_NONE) {
            ...
            changeByMixedEdgeType(edge, start, end);
    ...
    private void changeByMixedEdgeType(EdgeItem edge, Point2D start, Point2D end) {
        ...
        if (opposingEdge != null) {
            ...
            if (mixedEdgeType == MixedGraphEdgeType.Two_Edges) {
                ...
                double sourceX = sourceNodeBounds.getCenterX();
                double sourceY = sourceNodeBounds.getCenterY();
                double targetX = targetNodeBounds.getCenterX();
                double targetY = targetNodeBounds.getCenterY();
                ...
                double nX = -directionY / length;
                double nY = directionX / length;
                ...
                start.setLocation(
                        start.getX() + nX * (sourceNodeBounds.getWidth() / 2) * offsetStrength,
                        start.getY() + nY * (sourceNodeBounds.getHeight() / 2) * offsetStrength);
                end.setLocation(end.getX() + nX * (targetNodeBounds.getWidth() / 2) * offsetStrength,
                        end.getY() + +nY * (sourceNodeBounds.getHeight() / 2) * offsetStrength);
            } else if (mixedEdgeType == MixedGraphEdgeType.Half_Edges) {
                float directionX = (float) (end.getX() - start.getX());
                float directionY = (float) (end.getY() - start.getY());
                end.setLocation(start.getX() + (directionX / 2), start.getY() + (directionY / 2));
            }
        }
    }
    ...
    public enum MixedGraphEdgeType {
