 ...
 * current members.</p>
 *
 * <p>Updating a focus group does not necessarily cause the display to change.
 * For this functionality, either register an action with this control, or
 * register a TupleSetListener with the focus group.
 * </p>
 *
...
public class FocusControl extends ControlAdapter {
    ...
    private boolean focusOnEnter = false;
    private boolean wasInFocusGroup = false;
    private boolean itemFixedByClick = false;
    ...
    public void itemEntered(VisualItem item, MouseEvent e) {
        ...
        if (ccount == 0 || focusOnEnter) {
            Visualization vis = item.getVisualization();
            TupleSet ts = vis.getFocusGroup(group);
            if (ccount == 0) {
                ts.setTuple(item);
            } else {
                wasInFocusGroup = ts.containsTuple(item);
                if (!wasInFocusGroup) {
                    ts.addTuple(item);
                }
            }
    ...
    public void itemExited(VisualItem item, MouseEvent e) {
        ...
        if (ccount == 0 || focusOnEnter) {
            ...
            Visualization vis = item.getVisualization();
            TupleSet ts = vis.getFocusGroup(group);
            if (!wasInFocusGroup && !itemFixedByClick) {
                ts.removeTuple(item);
            }
            runActivity(vis);
        }
        wasInFocusGroup = false;
        itemFixedByClick = false;
    }
        ...
        if (UILib.isButtonPressed(e, button)
            ...
            if (item != curFocus) {
                ...
                if (!ctrl) {
                    curFocus = item;
                    ts.setTuple(item);
                    itemFixedByClick = true;
                } else if (ts.containsTuple(item) && wasInFocusGroup) {
                    ts.removeTuple(item);
                } else {
                    ts.addTuple(item);
                    itemFixedByClick = true;
    ...
    /**
     * @return the focusOnEnter
     */
    public boolean isFocusOnEnter() {
        return focusOnEnter;
    }

    /**
     * @param focusOnEnter the focusOnEnter to set
     */
    public void setFocusOnEnter(boolean focusOnEnter) {
